...
def hexdump(data, columns=16, indentlvl=""):
    ...
    def do_line(line):
        return (
            indentlvl +
            " ".join("{:02x}".format(b) for b in line) +
            "   " * (columns - len(line)) +
            "  " +
            "".join(chr(b) if 32 <= b < 127 else "." for b in line))
...
class ENIP_ConnectionPacket(scapy_all.Packet):
    ...
    fields_desc = [
        scapy_all.LEShortField("sequence", 0)
    ]
...
class ENIP_SendUnitData(scapy_all.Packet):
    ...
    fields_desc = [
        ...
        scapy_all.PacketListField("items", [], ENIP_SendUnitData_Item, count_from=lambda p: p.count)
    ]
